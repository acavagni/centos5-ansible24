...
# use Docker containers to quickly spin up multiple CentOS-5 environments 
...
#
# New sshd server keys are generated at runtime (on each container run) so you should
# add the "-o UserKnownHostsFile=/dev/null" to the ansible.cfg ssh_args parameter
# to make login possible. BE CAREFUL: THIS IS INTENDED FOR TESTING PURPOSE ONLY!!!
#
